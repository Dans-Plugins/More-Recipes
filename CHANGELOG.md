...
- `CONTRIBUTING.md` now describes the unit-test suite — `mvn test`, where tests live and how they mirror the main packages, that they run in CI on every pull request — instead of a Testing section written before the suite existed, and `README.md` now links the user guide, command reference, configuration reference, contributing guide and changelog, none of which were reachable from it.
- Bare `/mr` is now covered by a test: the banner carries the plugin's version, the credits and wiki lines follow it, nothing else is sent, and passing arguments changes none of that. It was the one command without a test. The `/mr help` test now asserts the full `/mr get` line, so the corrected wording (`Get a certain amount`, was `Get an certain amount`) cannot quietly regress; the previous assertion matched either. No command changed.
- `COMMANDS.md` now lists bare `/mr` (version, authors and wiki link; no permission checked), which the README describes the page as covering but which was missing, and says that `/mr get` is refused from the console, what `<itemName>` and `<amount>` accept, and what happens to items that don't fit. `USER_GUIDE.md` likewise says `/mr get` is in-game only, and the comment in the bundled `config.yml` now says a `command` event carries the command name rather than only the plugin version. No behaviour changed.
- `USER_GUIDE.md` now has a Recipes table giving each item's crafting grid and how many items it yields, which no page described before — the guide named the items but not what crafts them. A test compares the table with the recipes the plugin registers, so a changed ingredient, shape or yield fails the build until the table is updated. `COMMANDS.md` now says `/mr get` matches item names regardless of case. No recipe changed.
